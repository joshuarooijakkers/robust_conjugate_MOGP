...
class MOGPRegressor_NC_bounded_noise:
    def __init__(self, mean=0.0, length_scale=1.0, noise=1e-2, A=None):
        self.D = A.shape[0]
        self.mean = mean
        self.length_scale = length_scale
    ...
    def optimize_hyperparameters(self, print_opt_param=False, print_iter_param=False, bound=1e-3):
        def objective(theta):
            val = -self.log_marginal_likelihood(theta)
            if print_iter_param:
                print(-val)
        ...
        res = minimize(objective, initial_theta, method='L-BFGS-B',
                       bounds = [(np.log(1e-3), np.log(1e3)),     # length_scale
                                (np.log(bound), np.log(bound))] + [(np.log(1e-2), np.log(1e2))] * (self.A.shape[0] * self.A.shape[1])
        )
...
class MORCGP_bounded_noise:
    ...
    def optimize_loo_cv(self, print_opt_param = False, print_iter_objective=False, k=1, init_cov=None, fix_weights=True, bound = 1e-3):
